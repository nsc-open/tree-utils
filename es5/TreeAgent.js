...
class TreeAgent {
  ...
  _flatten(tree) {
    ...
    walk(tree, ({
      ...
      const currentNode = {
        ...
        parent: null,
        children: null,
        isLeaf: options.leafIndicatorPropName in node ? _isLeaf(node) : !node.children || node.children.length === 0
      };
  ...
  setFieldValue(key, fieldName, fieldValue, options = {
    cascade: false,
    beforeSet: null
  }) {
    const node = this.getNode(key);
    ...
    if (cascade) {
      _setCascadeField(node, true); // set value for children
        ...
        this.getParents(key).reverse().forEach(parent => {
          const indeterminate = !parent.children.every(child => {
            const {
              value,
              indeterminate
            } = this.getFieldValue(_key(child.node), fieldName, true);
  ...
  /* TODO */
    ...
    Object.values(subNodeMap).forEach(n => {
      ...
      if (_key(n.node) === key) {
        n.parent = parent; // set children of parent in both nodeMap and tree
      ...
      n.path = [...parent.path, ...n.path]; // add to the main nodeMap
